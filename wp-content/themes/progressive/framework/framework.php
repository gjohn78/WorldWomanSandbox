...
function ts_get_embaded_video($url, $return_url = false) {
	...
	if (strstr($url,'vimeo'))
	{
		if (preg_match('/(\d+)/', $url, $matches))
		{
			...
			return '<div class="video-box vimeo"><iframe src="http://player.vimeo.com/video/'.$matches[0].'?title=0&amp;byline=0&amp;portrait=0&amp;color=ffffff"></iframe></div>';
		}
	}
	else if (strstr($url,'youtu'))
	{
		...
		if (preg_match($pattern, $url, $matches))
		{
			...
			return '<div class="video-box youtube"><iframe src="http://www.youtube.com/embed/'.$matches[0].'"></iframe></div>';
		}
	}
	return '';
}
...
function ts_get_image($image_src, $class = '', $alt = '') {
	...
	if ($attachment_id) {
		...
		return '<img src="'.esc_url($image_src).'" class="'.sanitize_html_classes('class').'" alt="'.esc_attr($alt).'" />';
	}
}
 ...
 */
function ts_contact_form_submission()
{
	if ($_SERVER['REQUEST_METHOD'] != 'POST' && ! wp_verify_nonce( $_POST['_wpnonce'], 'my-contact-form' ) ) {
		die('ERROR NONCE');
	}
	
	$post = get_post((int)$_POST['your_email']);
	preg_match('/\[contact_form.*email=\"([^\"]*?)\"/i', $post -> post_content, $matches);
	
	if (!is_array($matches) || !isset($matches[1])) {
		die('ERROR EMAIL');
	}
	$your_email = $email = $matches[1];
	...
	$to      = $your_email;
	$subject = __('Contact Form', 'progressive');
	$headers = 'From: '.$name.' <'.$email.'>' . "\r\n";
	$message = $name.' '.__('sent you a message via the contact form', 'progressive'). ':'."\r\n".$message;
	
	wp_mail($to, $subject, $message, $headers);
	die('OK');
}
add_action( 'wp_ajax_nopriv_contact_form_submission', 'ts_contact_form_submission' );
add_action( 'wp_ajax_contact_form_submission', 'ts_contact_form_submission' );
...
/**
 * Replace emails with mailto: link
 * @param string $text
 * @return strings
 */
function ts_emailize($text)
{
