...
jQuery(document).ready(function($) {
	...
	<?php if (!empty($data_field)): ?>
		...
		jQuery('#TB_overlay,.tb-close-icon').click(function() {
			...
			if (jQuery('#pb_item_data_<?php echo esc_js($data_field); ?>').length > 0 && jQuery('#pb_item_data_<?php echo esc_js($data_field); ?>').val() == "")
			{
				jQuery('#pb_item_data_<?php echo esc_js($data_field); ?>').closest('.pb_item_wrapper').remove();
			}
		});
	<?php endif;?>
	...
	processShortcode = function() {
		...
		if (levels[0] !== undefined)
		{
			<?php if (!empty($data_field)): ?>
				...
				pb_tmp_data = jQuery.parseJSON(jQuery('#pb_item_data_<?php echo esc_js($data_field);?>').val());
	...
	jQuery('#insert-shortcode').click(function(){
		...
		if (!empty($callback)): ?>
			<?php echo esc_js($callback).'("'.esc_js($data_field).'", "'.esc_js($field_id).'", pb_data);'; ?>
		<?php elseif (!empty($data_field)): ?>
			pb_tmp_data = {};
			pb_tmp_data['shortcode'] = shortocdeFinalString;
			pb_tmp_data['data'] = pb_data;

			jQuery('#pb_item_data_<?php echo esc_js($data_field); ?>').val(JSON.stringify(pb_tmp_data));
		<?php else: ?>
